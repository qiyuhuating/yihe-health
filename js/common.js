...
function saveJSON(key, val){try{localStorage.setItem(key,JSON.stringify(val));}catch(e){console.warn('localStorage write failed:',key,e.message);showToast('保存失败，请检查浏览器存储空间');}}
...
function loadUserData(base, uid, def) {
  var k = userKey(base, uid);
  var v = loadJSON(k, null);
  if (v == null) {
    var legacy = loadJSON(base, null);
    if (legacy != null) { v = legacy; saveJSON(k, v); try { localStorage.removeItem(base); } catch (e) {} }
  }
  return v == null ? def : v;
}
...
/* 新中式首字头像 — 姓名哈希选色 + 毛笔首字 */
var AVATAR_COLORS = [
  {c:'#9E3B2E',d:'#7A2D22'},{c:'#5E7B57',d:'#46613F'},{c:'#B07A2E',d:'#8A6022'},
  {c:'#4A6B8A',d:'#344E66'},{c:'#7B5A8C',d:'#5A3F66'},{c:'#8A5A44',d:'#66442F'},
  {c:'#5A7B8A',d:'#3F5A66'},{c:'#8A6B3D',d:'#66502A'}
];
...
function firstCharOf(name) {
  ...
  for (var i = 0; i < prefixes.length; i++) { if (s.indexOf(prefixes[i]) === 0) { s = s.substring(prefixes[i].length); break; } }
  s = s.trim(); if (!s) return name.trim().charAt(0) || '用';
  return s.charAt(0);
}
...
function avatarSVGForRole(role, size) {
  var map = { doctor: '张医生', nurse: '王护士', family: '家属' };
...
function getLoginName() {
  try { if (window.App && App.loginInfo && App.loginInfo.name) return App.loginInfo.name; } catch (e) {}
  try { var raw = localStorage.getItem('hm-login'); if (raw) { var o = JSON.parse(raw); if (o && o.name) return o.name; } } catch (e) {}
  return '';
}
...
function renderUserCalligraphyAvatar() {
  ...
  if (!wrap) return;
  var old = wrap.querySelector('.calligraphy-avatar'); if (old) old.parentNode.removeChild(old);
  var nm = getLoginName() || '用';
  var box = document.createElement('div'); box.className = 'calligraphy-avatar'; box.innerHTML = calligraphyAvatar(nm);
  wrap.appendChild(box); wrap.classList.add('has-calligraphy');
  ...
  if (img) img.classList.remove('show'); if (fb) fb.classList.add('hidden');
  ...
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(function () {
      ...
      if (w && w.querySelector('.calligraphy-avatar')) {
        var o = w.querySelector('.calligraphy-avatar'); if (o) o.parentNode.removeChild(o);
        var b = document.createElement('div'); b.className = 'calligraphy-avatar'; b.innerHTML = calligraphyAvatar(nm);
        w.appendChild(b);
      }
    });
  }
