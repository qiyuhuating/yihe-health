...
(function () {
  ...
  function arrow() { return window.siteIcon ? window.siteIcon("arrowRight", 18) : "›"; }
  function back() { return window.siteIcon ? window.siteIcon("arrowRight", 18) : "‹"; }
  ...
  function render() {
    ...
    var art = null;
    for (var i = 0; i < all.length; i++) if (all[i].id === id) { art = all[i]; break; }
    ...
    if (rel) {
      var others = all.filter(function (x) { return x.id !== art.id; });
      var same = others.filter(function (x) { return x.cat === art.cat; });
      var rest = others.filter(function (x) { return x.cat !== art.cat; });
